"""
Render-em Native PPTX Toolkit
===============================
Reusable helpers for generating editable PowerPoint slides with:
- Brand theme loading from ~/.render-em/themes/
...
class RenderEmNative:
    ...
    def __init__(self, theme_path=None):
        """Initialize with optional theme. Auto-loads from ~/.render-em/themes/ if not specified."""
    ...
    def _load_theme(self, theme_path):
        """Load theme from file or auto-detect from ~/.render-em/themes/."""
        if theme_path and os.path.exists(theme_path):
            with open(theme_path) as f:
                self.theme.update(json.load(f))
            return

        themes_dir = os.path.expanduser("~/.render-em/themes")
